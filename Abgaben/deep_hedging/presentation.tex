...
\begin{frame}[t]
    ...
    \only<4>{
        Note that for hedging strategies we have the requirement that
        $$\delta_{-1} = \delta_T = 0$$
    }
\end{frame}
...
\begin{frame}
    ...
    \]
    \only<2>{
        \vs This however is a notoriously hard problem to solve since
        the optimization runs over all possible trading strategies $\delta \in \H$.
    }
\end{frame}
